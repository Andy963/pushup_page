...
class CircularDrawer(TracksDrawer):
    ...
    def _determine_ring_distance(self) -> Optional[int]:
        length_range = self.poster.length_range_by_date
        ring_count = None
        for count in [10, 20, 50, 100]:
            if length_range.upper() < count:
                continue
            ring_count = count
            if (length_range.upper() / count) <= 5:
                break
        return ring_count

    def _draw_rings(self, dr: svgwrite.Drawing, center: XY, radius_range: ValueRange):
        length_range = self.poster.length_range_by_date
        ring_count = self._determine_ring_distance()
        if ring_count is None:
            return
        count = ring_count
        while count < length_range.upper():
            radius = (
                radius_range.lower()
                + radius_range.diameter() * count / length_range.upper()
            )
            dr.add(
                dr.circle(
                    center=center.tuple(),
