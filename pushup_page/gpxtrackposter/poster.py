...
class Poster:
    ...
    def __init__(self):
        ...
        self.year_tracks_date_count_dict = {}
    ...
    def set_tracks(self, tracks):
        ...
        for track in tracks:
            if not self.years.contains(track.start_time() + self.tc_offset):
                continue
            text_date = (track.start_time() + self.tc_offset).strftime("%Y-%m-%d")
            if text_date in self.tracks_by_date:
                self.tracks_by_date[text_date].append(track)
            else:
                self.tracks_by_date[text_date] = [track]
            self.length_range.extend(track.count)
    ...
    def draw(self, drawer, output):
        ...
        g = d.g()
        d.add(g)
        g.add(d.rect((0, 0), (width, height), fill=self.colors["background"]))
    ...
    def __draw_footer(self, d):
        ...
        d.add(
            d.text(
                self.trans("Count") + f": {len(self.tracks)}",
        ...
        d.add(
            d.text(
                self.trans("Weekly") + ": " + format_float(weekly_count),
        ...
        d.add(
            d.text(
                self.trans("Total") + ": " + str(total_count),
        ...
        d.add(
            d.text(
                self.trans("Avg") + ": " + str(int(avg_monthly_count)),
        ...
        d.add(
            d.text(
                self.trans("Max") + ": " + str(int(max_monthly_count)),
                insert=(180, self.height - 15),
                fill=text_color,
                style=small_value_style,
            )
        )
    ...
    def __compute_track_statistics(self):
        count_range = ValueRange()
        total_count = 0
        total_count_year_dict = defaultdict(int)
        ...
        return (
            total_count,
            count_range.lower() or 0,
            count_range.upper() or 0,
            len(self.tracks) / len(tracks_by_month) if tracks_by_month else 0,
            avg_monthly_count,
            max_monthly_count,
        )
    ...
    def __compute_years(self, tracks):
        ...
        for t in tracks:
            self.years.add(t.start_time() + self.tc_offset)
